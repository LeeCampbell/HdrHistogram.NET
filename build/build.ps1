...
#	Reqires powershell 5 (https://www.microsoft.com/en-us/download/details.aspx?id=50395)
#	Requires 
...
#		-No dependecy on 7Zip. So the binary is not checked into source control
#		-Nuget is not in source control. We look for an installed version, else download it
#		-Currently no support for dotNetCore so no depenedency on KVM (which I think is deprecated now anyway, in favor of dotnet cli)
...
task VerifyDependencies {
	if($PSVersionTable.PSVersion.Major -lt 5) {
		#Compress-Archive and Expand-Archive are PS5 feature. Means we dont need a binary dependency on 7Zip or have to hand code .NET or Shell compression.
